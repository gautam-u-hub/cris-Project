...
if ($result2->num_rows > 0) {
  ...
  echo "first add previous month's values";
  return;
}
if(isset($cc)){
  ...
  for($i=0;$i<=4;$i++)
{
  ...
  $c=(float)$cc[$i]/$Target_Year[$i]*100;
    $s=(float)$c*5;
  $sql = "INSERT INTO `kpi`(MONTH,YEAR,ZONE,DIVISION,target,performance,cummilative,max,achievement,score) VALUES ('$MONTH','$YEAR','$zones1','$division','$Target_Year[$i]','$performance[$i]','$cc[$i]',5,'$c','$s')";
  ...
  for($i=5;$i<=9;$i++)
{
  ...
  $sql = "INSERT INTO `kpi`(MONTH,YEAR,ZONE,DIVISION,target,performance,cummilative,max,achievement,score) VALUES ('$MONTH','$YEAR','$zones1','$division','$Target_Year[$i]','$performance[$i]','$cc[$i]',4,'$c','$s')";
    if ($con->query($sql) === TRUE) {
     // echo "New record created successfully 22";
    } else {
     // echo "Error: " . $sql . "<br>" . $con->error;
    }
  ...
  for($i=10;$i<=13;$i++)
  {
    ...
    $c=(float)$cc[$i]/$Target_Year[$i]*100;
    $s=(float)$c*3;
  $sql = "INSERT INTO `kpi`(MONTH,YEAR,ZONE,DIVISION,target,performance,cummilative,max,achievement,score) VALUES ('$MONTH','$YEAR','$zones1','$division','$Target_Year[$i]','$performance[$i]','$cc[$i]',3,'$c','$s')";
    if ($con->query($sql) === TRUE) {
     // echo "New record created successfully 2";
    } else {
     // echo "Error: " . $sql . "<br>" . $con->error;
